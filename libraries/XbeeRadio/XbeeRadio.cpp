...
// get SL
	...
	Serial.println("J'envoie ma premiere requete");
...
// send the command
Serial.println("je m'apprette à emmetre au XBEE");
	this->send(atRequest);
	Serial.println("requete émise au XBEE");
// wait up to 5 seconds for the status response
	if (this->readPacket(5000))
	{
	// should be an AT command response
	Serial.println("j'ai reçu une réponse a ma requete");
	...
	{
		Serial.println("Il n'avait pas repondu !!");
		error=1;
	}
...
void XBeeRadio::initialize_xbee_module(long baudrate)
{
 ...
 // DBG(Serial.println(cur_baud);)
  ...
  setup_command("ATBD5");
 // setup_command("ATDH0");
    setup_command("ATMY06");
   setup_command("ATIDFFFF");
Serial.println("ici");
  setup_command("ATDLFFFF");
    setup_command("ATWR");
  setup_command("ATAC");
  Serial1.end();
  digitalWrite(13, LOW);
...
//  = "ATMM2\r";
  bool ret_val;
  for(int i = 0; i < 5; i++) //essayer d'envoyer la commande 5 fois
  {
	  Serial.println(strlen(bla));
    for(int k = 0; k < strlen(bla); k++)
    {
      Serial1.print(bla[k]);
      delay(200);
    }
...
bool XBeeRadio::check_for_response(void)
{
  ...
  int j = 0;
  while(Serial1.available() && j < (BUFFER_SIZE-1))
  {
      int tmpChar = Serial1.read();
