...
void TreeRouting::loop() {
    if (xbee->checkForData(112)) {
        ...
        uint8_t rssi = rx.getRssi(); // Puissance du signal
    ...
    if (state_ == TrGateway) {
        ...
        if (millis() - lastBeacon > 3000) { // renvoi du broadcast d'invitation
            timer_elapsed();
            lastBeacon = millis();
        }
    } else {
        if (millis() - lastBeacon > 10000) { // deconnexion avec le parent
...
void TreeRouting::receive(uint16_t from, byte * payload, unsigned int len, uint8_t metric) {
    ...
    if (msg_id == TrMsgIdBroadcast) {
        ...
        if (!is_known(from)) { // verifier que ce n'est pas l'un des fils

            if (state_ != TrGateway) {
                //set new or better gateway
                if ((hops_ == 0xff) || (parent_lqi_ > metric)) { // pas de parent ou meilleur parent par qualité de lien et pas nombre de sauts
                    ...
                    radio_send(0xffff, (byte*) message, message->buffer_size()); //rebroadcast
                }
                //update the gateway's life indicator
                if ((state_ == TrConnected) && (parent_ == from)) {  // !! ne va t'il pas le renvoyer deux fois le message de broadcast si il vient de choisir le pere ?
                    lastBeacon = millis();
                    message->set_hops(hops_);
        ...
        if (state_ == TrGateway) { // mon fils me dit qu'il a un fils ?
            //message reached its destination
            received_callback_(message->source(), message->payload(), message->payload_size());
        } else if (parent_ != 0) { // je n'ai pas perdu mon pere en attendant ?
            add_child(message->source());
            radio_send(parent_, payload, len);
        }
    } else if (msg_id == Tr2MsgIdRouting) {
        TreeRoutingMessage *message = reinterpret_cast<TreeRoutingMessage*> (payload);
        if (message->source() == this->myAddress) { // mon père me dit que mon grand père à forwarder mon message 
            //message reached its destination
            received_callback_(message->source(), message->payload(), message->payload_size());
